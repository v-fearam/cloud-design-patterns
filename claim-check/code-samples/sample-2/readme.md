...
Using Event Hubs enables integration with existing Event Hubs-based solutions and lets you take advantage of capabilities like schema registry support and Apache Kafka compatibility. Event Hubs can also be configured to [automatically archive](https://learn.microsoft.com/azure/event-hubs/event-hubs-capture-overview) received messages, making them available as Avro files for integration with tools like Apache Spark, Stream Analytics, and Azure Data Factory.
...
![A diagram showing Event Grid connected to Azure Blob Storage. As blobs are created, Event Grid forwards a message containing the reference to the blob to an Event Hub. A consumer CLI application receives the message from Event Hubs, extracts the reference, and downloads the blob from the storage account.](images/sample-2-diagram.png)

1. The payload is uploaded to Azure Blob Storage.
...
1. A .NET console program connects to the Event Hub and reads messages as they arrive.
